...
def resize_image(matrix: list, new_width = 64, new_height = 32):
    ...
    for y in range(height):
        for x in range(width):
            ...
            first_pixel = matrix[y][x]
            secound_pixel = matrix[y][x+1]
            ...
            curvecure_r = (matrix[y+1][x+1][0]-matrix[y+1][x][0])/(x_scaling_factor-1)
            curvecure_g = (matrix[y+1][x+1][1]-matrix[y+1][x][1])/(x_scaling_factor-1)
            curvecure_b = (matrix[y+1][x+1][2]-matrix[y+1][x][2])/(x_scaling_factor-1)

            current_pixel = matrix[y+1][x]
